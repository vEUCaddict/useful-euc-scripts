...
# DISABLE DEFENDER SECURITY CENTER COMPLETLEY IN WINDOWS 10/11 (USED FOR APP VOLUMES CAPTURE VM)
...
New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender Security Center\Enterprise Customization" -Name CompanyName -PropertyType String -Value "LANOS-GreenIT" | Out-Null
New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender Security Center\Enterprise Customization" -Name Phone -PropertyType String -Value "0000000000" | Out-Null
New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender Security Center\Enterprise Customization" -Name Url -PropertyType String -Value "https://support.lanos-greenit.nl" | Out-Null
